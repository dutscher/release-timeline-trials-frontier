...
export function getLocaleDateString() {
    ...
    const format = formats[navigator.language] || 'dd/MM/yyyy';

    return {
        format,
        separator: /[.-/]/.exec(format)[0],
    };

}

export function localeDateWithPad(timestamp) {
    // 2.2.2020 -> 02.02.2020
    const locale = getLocaleDateString();
    return timestamp
        .toLocaleDateString()
        .split(locale.separator)
        .map(number => number.padStart(2, '0'))
        .join(locale.separator);
}

export function getSplittedLocaleDate(germanDate) {
    // german date: dd.mm.yyyy
    const splittedDate = germanDate.split('.').map(number => parseInt(number));
    return new Date(splittedDate[2], splittedDate[1] - 1, splittedDate[0]);
}

export function toLocaleDate(germanDate) {
    return localeDateWithPad(getSplittedLocaleDate(germanDate));
}